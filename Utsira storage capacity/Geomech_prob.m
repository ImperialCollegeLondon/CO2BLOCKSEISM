function [deltap_cr,deltap_cr_ref,r_p,r_Sv,r_Shmin,r_SHmax,r_SHmax_dir,r_fault_mu,...
    ...
    if strcmp(f_dist_p,'Uni')
        pd_p = makedist('Uniform','lower',pres_offset+depth_seismic*(pres_grad_seismic - a_p_grad/1000),...
            'upper',pres_offset+depth_seismic*(pres_grad_seismic + a_p_grad/1000)) ; %#ok<LLMNC>
    ...
    if strcmp(f_dist_Sv,'Uni')
        pd_Sv = makedist('Uniform','lower',Sv_offset+depth_seismic*(Sv_grad_seismic - a_Sv_grad/1000),...
            'upper',Sv_offset+depth_seismic*(Sv_grad_seismic + a_Sv_grad/1000)) ; %#ok<LLMNC>
    ...
    if strcmp(f_dist_Shmin,'Uni')
        pd_Shmin = makedist('Uniform','lower',Shmin_offset+depth_seismic*(Shmin_grad_seismic - a_Shmin_grad/1000),...
            'upper',Shmin_offset+depth_seismic*(Shmin_grad_seismic + a_Shmin_grad/1000)) ; %#ok<LLMNC>
    ...
    if strcmp(f_dist_SHmax,'Uni')
        pd_SHmax = makedist('Uniform','lower',SHmax_offset+depth_seismic*(SHmax_grad_seismic - a_SHmax_grad/1000),...
            'upper',SHmax_offset+depth_seismic*(SHmax_grad_seismic + a_SHmax_grad/1000)) ;  %#ok<LLMNC>
    ...
    for i= 1:nr_fault
        [Sigma_n_ref,Tau_ref] = stress_projection(SHmax_dir_seismic,...
         ...
         % critical pressure to cause slip for MC realizations
        deltap_cr_ref(i) = Sigma_n_ref - (pres_offset + depth_seismic*pres_grad_seismic) ...
