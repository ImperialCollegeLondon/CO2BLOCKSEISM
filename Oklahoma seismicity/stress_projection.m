...
function [Sigma_n,Tau] = stress_projection(delta,SH_max,Sh_min,Sv,Azi,Dip)
    ...
    L_PG = [cos(delta) sin(delta) 0;...         % delta is the rotation angle from SHmax to the N direction
            sin(delta) -cos(delta) 0;...        % Rotation matrix: principal stress (SH-Sh-Sv) to geographical system (N-E-D)
    ...
    n_n = [-(sin(Azi))*(sin(Dip));...           % normal of the fault (in N-E-d system)
           (cos(Azi))*(sin(Dip));...            % normal to the footwall
           -cos(Dip)];

    n_s = [cos(Azi) ; sin(Azi) ; 0];            % strike direction (in N-E-d system)

    n_d = [-(sin(Azi))*(cos(Dip));...           % dip direction (in N-E-d system)
           (cos(Azi))*(cos(Dip));...            % always downward on the fault plane (third component +)
           sin(Dip)];                                    % dip direction of the footwall
